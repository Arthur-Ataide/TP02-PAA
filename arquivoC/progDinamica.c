...
int verificaPosicaoInicialFinal(PCaverna caverna){
    ...
    for (int i = entrada.linha; i >= saida.linha; i--){
        for (int j = entrada.coluna; j >= saida.coluna; j--){
            ...
            if (i == entrada.linha && j == entrada.coluna){
                caverna->MatrixDinamica[i][j] = caverna->vida;
...
void Caminho(PCaverna caverna, ParOrdenado* caminho){ // descobrir o caminho
    FILE * file;
